...
(async()=>{
 ...
 const [review]=await Promise.all([c.waitForEvent('page'),p.locator('#quick-save').click()]); await review.waitForLoadState();
 const draftBefore=await p.evaluate(()=>Pema.patient('P001').quickOrders.filter(x=>x.status==='draft').length);
 await review.locator('#approve').click(); await p.reload();
