...
7. Khi sửa provider `@riverpod` trong `lib/`: chạy `dart run build_runner build --delete-conflicting-outputs` và commit file `*.g.dart` sinh ra. Khi Dart/assets thay đổi: `dart analyze` (chạy cả `riverpod_lint`; `flutter analyze` không chạy plugin này), `flutter test --reporter expanded`, build preview; xem trực tiếp các màn bị ảnh hưởng. Review ở 360×800, 390×844, 430×932, 768×1024; suite hiện tại dùng height 844 ở cả bốn width. Không suy test tự động bằng test thiết bị.
