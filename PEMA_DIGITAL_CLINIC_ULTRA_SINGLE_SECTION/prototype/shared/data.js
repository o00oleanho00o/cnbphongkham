...
(function () {
  ...
  function initial() {
    return {
      ...
      patients: names.map((name,i)=>{
        ...
        const sessions = Array.from({length:completed},(_,j)=>({id:`s${i}-${j}`,date:j===completed-1?visitDate:`2026-0${7+Math.min(j,1)}-${String(12+j*6).padStart(2,'0')}`,type:g[2],note:j===completed-1?'Đã ghi nhận đáp ứng và gửi hướng dẫn chăm sóc.':'Buổi điều trị theo kế hoạch; đã lưu ảnh mốc.',reviewed:true}));
  ...
  if (!state || state.version!==2) state=initial();
  const P = window.Pema = {
    ...
    reload(){try {const s=JSON.parse(localStorage.getItem(KEY));if(s&&s.version===2&&Array.isArray(s.patients)&&Array.isArray(s.followups)) state=s}catch(_){}},
    ...
    date(s){return new Date(s).toLocaleDateString('vi-VN')},
    ...
    addEvent(p,kind,title,detail,by='BS. Tâm'){p.events.unshift({id:'e'+Date.now(),date:'2026-09-20',kind,title,detail,by})},
